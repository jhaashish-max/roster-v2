        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', marginBottom: '1.25rem' }}>
            ...
            <div style={{ background: 'var(--bg-card)', borderRadius: '12px', border: '1px solid var(--border-color)', padding: '1rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                {(() => {
                  // Only count rows with a known status, normalize case to merge "Done"/"DONE"
                  const statusMap = {};
                  knownProjects.forEach(p => {
                    const s = p.status.trim();
                    // Normalize to title-case key so "Done" and "DONE" merge
                    const key = s.charAt(0).toUpperCase() + s.slice(1).toLowerCase();
                    statusMap[key] = (statusMap[key] || 0) + 1;
                  });
                  const total = knownProjects.length || 1;
